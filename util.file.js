...
module.exports = function (waw) {
	...
	const serve_file = function (opts) {
		return function (req, res) {
			if (
				fs.existsSync(
					opts.dirname + req.params.file + "/" + req.params.size
				)
			) {
				res.sendFile(
					opts.dirname + req.params.file + "/" + req.params.size
				...
				if (
					...
					fs.existsSync(opts.dirname + opts.default)
				) {
					res.sendFile(opts.dirname + opts.default);
				} else if (opts.default) {
					console.log(
						"To change default picture you need to write specific name and put such file on the part."
					);
	...
	const serve = function (part, opts) {
		...
		waw.app.get(
			"/api/" + part + "/resized/:file/:size/:name",
			serve_file(opts)
		);
	...
	const upload_resized = function (req, res, part, opts) {
		let dirname =
			opts.dirname +
			req.body.file.split(".jpg")[0].split(".png")[0] +
			"/";
		fs.mkdirSync(dirname, { recursive: true });
		waw.dataUrlToLocation(
			req.body.dataUrl,
			dirname,
			req.body.name + ".jpg",
			() => {
			...
			const complete = (name) => {
				...
				req.files.push({
					url:
						...
						part +
						"/file/" +
						path.basename(file.filepath + "_" + name) +
						"/" +
						file.originalFilename.split(" ").join(""),
	...
	const manage = function (part, opts) {
		waw.app.post(
			"/api/" + part + "/file" + ((opts.name && "/" + opts.name) || ""),
			waw.middleware(opts.ensure || waw.role("admin")),
			function (req, res) {
		...
		waw.app.post(
			"/api/" + part + "/resize" + ((opts.name && "/" + opts.name) || ""),
			waw.middleware(opts.ensure || waw.role("admin")),
			function (req, res) {
	...
	waw.file = function (part, config) {
		...
		if (!options[part]) {
			return console.log(
				"Something went wrong, part was not executed correctly."
			);
		}
		if (set[part]) {
			return console.log(
				"You can't set same part multiple times. Please use once array of config's."
			);
		}
		set[part] = true;
